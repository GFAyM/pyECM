...
def test_ecm_and_epv_molecular_contributions(tmp_path, capfd):
    ...
    # Calculate Epv using the DHF orbitals
    mymolecule.Epv()
    epv_from_orbitals = mymolecule.Epv_expval

    # Calculate Epv using the density matrix constructed
    # from the DHF orbitals
    density_matrix = mymolecule.rel_pyscf.make_rdm1()
    mymolecule.Epv(dm=density_matrix)
    epv_from_dm = mymolecule.Epv_expval
 ...
 1.00428576, 1.00428576, 1.66399638, 1.66399638, 0.2041611,  0.2041611])
    ...
    assert_almost_equal(np.sum(epv_from_orbitals, axis=1)[:4],test_Epv_molcontr_Epv_sumovernuclei,decimal=4,)
    assert_almost_equal(np.sum(epv_from_dm, axis=1)[:4],test_Epv_molcontr_Epv_sumovernuclei,decimal=4,)
...
def test_website_interface(tmp_path, capfd):
    ...
    MOL='AP1'
    export_dir = main_directory + 'pyECM/data/export/'
    downloaded_chiral = export_dir + MOL + '_chiral.xyz'
    downloaded_achiral = export_dir + MOL + '_achiral.xyz'



    try:
        obtain_nearest_structure(MOL,os.path.abspath(main_directory+'pyECM/data/import/'),output_path=export_dir)

    # Verify if the structure downloaded from the csm website matches
    # the local one (API_chiral.xyz), which in turns is used for
    # the test_ecm_importing_achiral test. Here only the 
    # acquisition stage is tested.
        downloaded = molecula(XYZ_file=downloaded_chiral)
        reference = molecula(XYZ_file=main_directory + 'pyECM/data/import/' + MOL + '_chiral.xyz')

        out, err = capfd.readouterr()  # For possible test on stdout

        assert_almost_equal(downloaded.positions, reference.positions, decimal=4)

    # As data/export/ is shared by other tests and for interactive
    # use in the project, the generated files are here removed.
    finally:
        for f in (downloaded_chiral, downloaded_achiral):
            try:
                os.remove(f)
            except OSError:
                pass
