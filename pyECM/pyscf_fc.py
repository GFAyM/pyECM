...
def fc_integrals(mol, mf, atom, **kwargs):
    ...
    :type atom: int
    :return: Matrix of Fermi Contact integrals
             (4-component spinor form) for the specified atom.
    ...
    fc_integrals[:n2c, :n2c] = numpy.einsum(
        "ip,iq->pq", ao_spinor.conjugate(), ao_spinor
    )
    fc_integrals[:n2c, n2c:] = numpy.einsum(
        "ip,iq->pq", ao_spinor.conjugate(), ao_spinor_S
    ) * (0.5 / c)
    fc_integrals[n2c:, :n2c] = numpy.einsum(
        "ip,iq->pq", ao_spinor_S.conjugate(), ao_spinor
    ) * (0.5 / c)
    fc_integrals[n2c:, n2c:] = numpy.einsum(
        "ip,iq->pq", ao_spinor_S.conjugate(), ao_spinor_S
    ) * ((0.5 / c) ** 2)
...
def Epv_atom(mol, mf, atom_index, dm=None):
    ...
    if dm is not None:
        ...
        expval_perorb = numpy.einsum("ij,ji->i", gamma5_fc_mo, dm_mo)
...
def Epv_molecule(mol, mf, dm=None):
    ...
    :param mol: Molecule object containing information about the molecular system.
    :type mol: pyscf.gto.Mole
    :param mf: Mean-field object, typically obtained from a
               Dirac-Hartree-Fock (DHF) calculation.
    :type mf: pyscf.scf.hf.SCF
    :return: A 2D array where each row corresponds to an atom
             and each column contains the PV contributions
